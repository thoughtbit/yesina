...
public class TimeMeter {
    ...
    public static class TimePriceUnit {
         ...
         * @return TimePriceUnit实例
         */
        public static TimePriceUnit instanceBySettlement(ValuationRequestTask requestTask, int carId, String cityCode) {
            TimePriceUnit unit = new TimePriceUnit();
            unit.chargingCategoryEnum = ChargingCategoryEnum.Settlement;
    ...
    public static TimePriceResult measure(TimeSlice totalSlice, TimePriceUnit unit) {
        ...
        int totalDays = (int) Math.ceil(totalSeconds / Duration.ofDays(1).getSeconds());
        ...
        for (int i = 0; i < totalDays; i++) {
            ...
            //距离费用
            double distance = 0;
            if (ChargingCategoryEnum.Forecast == unit.getChargingCategoryEnum()) {
                distance = BigDecimal.valueOf(durationSeconds).multiply(BigDecimal.valueOf(unit.getSpeed())).doubleValue();
            } else {
                try {
                    distance = unit.requestTask.requestDistance(unit.carId, unit.cityCode, currentSlice.getX(), currentSlice.getY()).getDistance();
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }

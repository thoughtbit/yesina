package com.lkmotion.yesincar.timingtask;
...
public class CheckRecordFileUrl {
    ...
    public void checkRecordFileUrlStart() {
        List<SecretVoiceRecords> secretVoiceRecords = new ArrayList<>();
            ...
            for(int i=0;i<size;i++) {
                ...
                try {
                    ...
                    if(StringUtils.isNotBlank(DownloadUrl)) {
                        ...
                        String fileName = a[0];
                        String uploadFailName = aLiConfig.getFilePath()+"\\"+fileName;
                        ...
                        if(judeFileExists(file)) {
                            try {
                                //下载录音文件
                                downLoadFromUrl(DownloadUrl,fileName,aLiConfig.getFilePath());
                            } catch (IOException e) {
                                e.printStackTrace();
                            }
                        }
                        String targetFile = ossConfig.getOssFileName()+"/"+fileName;
                        //上传OSS
                        ResponseResult responseResult = uploadService.uploadFileToOss(targetFile, file);
    ...
    public String getRecordFileUrl(String callId, Date callTime) throws Exception{
        ...
        IClientProfile profile = DefaultProfile.getProfile("cn-hangzhou", aLiConfig.getKeyId(), aLiConfig.getKeySecret());
        try {
        ...
        //对应的号池Key
        request.setPoolKey(aLiConfig.getPoolKey());
